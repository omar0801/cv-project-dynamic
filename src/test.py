...
# Template candidates (first existing wins)
JOB_TYPES = {
...
def resolve_cover_template(job_type_code: str) -> Path | None:
    # Prefer job-type specific, then generic
...
def run_app():
    ...
    # Job type
    ttk.Label(form, text="Job Type:").grid(row=0, column=0, sticky="e", padx=(0, 8), pady=4)
    job_type_var = tk.StringVar(value="EE")
    job_type_menu = ttk.Combobox(form, textvariable=job_type_var, values=list(JOB_TYPES.keys()), state="readonly", width=5)
